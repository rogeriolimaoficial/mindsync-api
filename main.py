from fastapi import FastAPI, Hefrom fastapi import FastAPI, Header, HTTPException
...
def export_live_csv():
    try:
        ...
        if not t_df.empty:
            ...
            merged = pd.merge(t_df, b_df, left_on=['user_id', 'date_key'], right_on=['user_id', 'recorded_date'], how='left')
...
def receive_telemetry(payload: TelemetryBatchPayload):
    ...
    for kb in payload.keyboard_events:
        if kb.total_keys > 0:  # Ignora 5s com zero dados
            try:
                dt = datetime.fromisoformat(kb.timestamp.split("+")[0].replace("Z", ""))
    ...
    block_time = now_utc.strftime("%Y-%m-%d %H:%M:00")
    ...
    """, (payload.user_id, payload.os_type.lower(), block_time, active_mins, avg_kpm, avg_err, avg_mouse_spd, avg_straight, now_utc.isoformat()))
    conn.commit()
    conn.close()
...
def save_bio(user_id, date_str, brand, rhr, hrv, total_s, deep_s, rem_s, rec):
    ...
    """, (user_id, date_str, brand, rhr, hrv, round(total_s/60, 1), round(deep_s/60, 1), round(rem_s/60, 1), rec, datetime.now(timezone.utc).isoformat()))
    conn.commit()
    conn.close()
    export_live_csv()

@app.post("/api/biometrics/apple-watch")
def ingest_aw(p: AppleWatchPayload, authorization: Optional[str] = Header(None)):
    ...
    save_bio(p.user_id, p.date, "Apple Watch", p.resting_heart_rate_bpm, norm_hrv, tot_s, int(p.deep_sleep_minutes*60), int(p.rem_sleep_minutes*60), rec)
    return {"status": "success"}

@app.post("/api/biometrics/whoop")
def ingest_whoop(p: WhoopPayload, authorization: Optional[str] = Header(None)):
    if authorization != f"Bearer {SECRET_API_TOKEN}": raise HTTPException(status_code=401, detail="Token inválido.")
    save_bio(p.user_id, p.date, "Whoop", p.resting_heart_rate_bpm, p.hrv_rmssd_ms, int(p.total_sleep_minutes*60), int(p.deep_slow_wave_sleep_minutes*60), int(p.rem_sleep_minutes*60), p.recovery_score_pct)
    return {"status": "success"}

@app.post("/api/biometrics/garmin")
def ingest_garmin(p: GarminPayload, authorization: Optional[str] = Header(None)):
    if authorization != f"Bearer {SECRET_API_TOKEN}": raise HTTPException(status_code=401, detail="Token inválido.")
    save_bio(p.user_id, p.date, "Garmin", p.resting_heart_rate_bpm, p.hrv_weekly_avg_ms, p.total_sleep_seconds, p.deep_sleep_seconds, p.rem_sleep_seconds, float(p.body_battery_max))
    return {"status": "success"}

@app.post("/api/biometrics/amazfit")
def ingest_amazfit(p: AmazfitPayload, authorization: Optional[str] = Header(None)):
    if authorization != f"Bearer {SECRET_API_TOKEN}": raise HTTPException(status_code=401, detail="Token inválido.")
    rec = float(p.readiness_score) if p.readiness_score else float(p.sleep_score)
    save_bio(p.user_id, p.date, "Amazfit", p.resting_heart_rate_bpm, p.hrv_score_ms or 0.0, int(p.total_sleep_minutes*60), int(p.deep_sleep_minutes*60), int(p.rem_sleep_minutes*60), rec)
    return {"status": "success"}

@app.post("/api/labels")
def ingest_label(p: GroundTruthPlaceholder):
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
    cursor.execute("INSERT INTO ground_truth_labels (user_id, timestamp, slot_1, slot_2, notes) VALUES (?, ?, ?, ?, ?)",
...
def dashboard_status():
    ...
    for u in all_users:
        cursor.execute("SELECT timestamp_block, os_type, count(id) FROM telemetry_15m WHERE user_id = ? ORDER BY id DESC LIMIT 1", (u,))
        t_row = cursor.fetchone()
        last_t = t_row[0] if t_row else None
        os_type = t_row[1].upper() if (t_row and t_row[1]) else "--"
        total_blocks = t_row[2] if t_row else 0

        cursor.execute("SELECT recorded_date, device_brand, recovery_score, hrv_rmssd_ms FROM daily_biometrics WHERE user_id = ? ORDER BY recorded_date DESC LIMIT 1", (u,))
        b_row = cursor.fetchone()
        bio_info = f"{b_row[1]} ({b_row[0]} | Rec: {b_row[2]}% | HRV: {b_row[3]}ms)" if b_row else "Sem Wearable"

        status_html = '<span style="color:#ef4444; font-weight:bold;">● Inativo / Fim de Semana</span>'
        last_seen = "Sem dados"
        if last_t:
            try:
                dt_last = datetime.strptime(last_t, "%Y-%m-%d %H:%M:%S").replace(tzinfo=timezone.utc)
                diff_min = (now_utc - dt_last).total_seconds() / 60.0
                last_seen = f"há {int(diff_min)}m" if diff_min < 120 else dt_last.strftime("%d/%m %H:%M")
                if diff_min <= 16:
    ...
    <head>
        ...
        <style>
            ...
            .btn {{ background: #0284c7; color: white; text-decoration: none; padding: 8px 16px; border-radius: 6px; font-size: 13px; font-weight: 600; }}
    ...
    <body>
        <div class="container">
            <div class="header">
                ...
                <a href="/api/dataset/download-csv" class="btn">⬇ Abrir Ficheiro CSV</a>
            </div>
            <table>
                <thead>
