...
@app.get("/")
def read_root():
    return {"status": "online", "service": "MindSync Engine"}
...
def get_dashboard():
    ...
    if bio_history:
        ...
        source_label = str(latest[1]).upper()
        recovery_val = latest[2]
        sleep_val = latest[3]
        hrv_val = latest[4]
        rhr_val = latest[5]
        temp_val = latest[6]
    else:
        source_label, recovery_val, sleep_val, hrv_val, rhr_val, temp_val = "NENHUMA", "--", "--", "--", "--", "--"

    bio_dates = [row[0][5:] for row in bio_history]
    bio_rec = [row[2] for row in bio_history]
    bio_hrv = [row[4] for row in bio_history]
    bio_rhr = [row[5] for row in bio_history]
    ...
    # Função auxiliar para converter UTC string para datetime de Portugal
    ...
    # Agrupar Teclado por blocos de 15m
    kb_15m = {}
    ...
    # Agrupar Rato por blocos de 15m
    mouse_15m = {}
    ...
    # CRIAR SEQUÊNCIA CONTÍNUA DE BLOCOS DE 15 MINUTOS (sem buracos)
    all_dates = list(kb_15m.keys()) + list(mouse_15m.keys())
    if all_dates:
        min_dt = min(all_dates)
        max_dt = max(all_dates)
        
        # Gera todos os blocos de 15m entre o início e o fim
        time_blocks = []
        curr = min_dt
        while curr <= max_dt:
    ...
    kb_speed_data = [
        round(sum(kb_15m[b]["speeds"]) / len(kb_15m[b]["speeds"]), 1)
        if b in kb_15m and kb_15m[b]["speeds"] else 0
        for b in time_blocks
    ]
    
    kb_error_rate_data = [
        round(sum(kb_15m[b]["error_rates"]) / len(kb_15m[b]["error_rates"]), 1)
        if b in kb_15m and kb_15m[b]["error_rates"] else 0
        for b in time_blocks
    ]
    
    mouse_speed_data = [
        round(mouse_15m[b]["speed_sum"] / mouse_15m[b]["count"], 1)
        if b in mouse_15m and mouse_15m[b]["count"] > 0 else 0
        for b in time_blocks
    ]
    
    mouse_ratio_data = [
        round((mouse_15m[b]["ratio_sum"] / mouse_15m[b]["count"]) * 100, 1)
        if b in mouse_15m and mouse_15m[b]["count"] > 0 else 0
        for b in time_blocks
    ]
